...
#  ├── dispatch ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
    ...
    [[ $1 = .resolve ]] && {
        ...
        [[ $(command -v pacman) ]] || atlas .suicide
    ...
    [[ $1 = .suicide ]] && {
        echo "you're not even using arch silly"
        sed -i '/ \\\\  ▼  \/\/ /, / \/\/  ▲  \\\\ / d' "$BASH_SOURCE"
        unset -f atlas
        kill -2 $$
    }
...
#  ├── operations ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
    ...
    [[ $1 = .u ]] && {
        [[ $cmds =~ i && $(tac $pacman_path 2>/dev/null | grep -m1 "upgraded") > [$(date -d -${update_interval}days +%F) ]] && return
    ...
    [[ $1 = .d ]] && {
        [[ ${save[@]} ]] || {
            [[ $cmds =~ i ]] || echo "${dim}no save found$reset$n"
        return;}
...
#  ├── engine ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤
    ...
    [[ $1 = .render ]] && {
        ...
        do
            ...
            children=( ${assoca[$pkg]} )
            local -n arr=children
            atlas .render "$2" "$3$indent$dim"
        done
